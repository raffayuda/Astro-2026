...
export const committeeDivisionsModule = new Elysia({ prefix: "/committee-divisions" })
  ...
  .get("/", () =>
    db
      ...
      .orderBy(asc(committeeDivisions.sortOrder), asc(committeeDivisions.id)),
  )
  .post(
    "/",
    async ({ body }) => {
    ...
    async ({ params, body }) => {
      try {
        ...
        if (body.shortName !== undefined)
          updates.shortName = body.shortName ? body.shortName.trim() : null;
        if (body.slug !== undefined && body.slug.trim() !== "") updates.slug = body.slug.trim();
        if ((body as any).sortOrder !== undefined)
          updates.sortOrder = (body as any).sortOrder === null ? 0 : (body as any).sortOrder;
        ...
        const item = await db.transaction(async (tx) => {
          if (isSlugChanged) {
            ...
            await tx.delete(committeeDivisions).where(eq(committeeDivisions.id, id));
  ...
  .delete(
    ...
    async ({ params }) => {
      ...
      await db.delete(committeeDivisions).where(eq(committeeDivisions.id, id));
      return { success: true };
    },
    {
      params: t.Object({ id: t.String() }),
