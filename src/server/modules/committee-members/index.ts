...
export const committeeMembersModule = new Elysia({ prefix: "/committee-members" })
  ...
  .post(
    ...
    async ({ body }) => {
      ...
      const rawDivisions = Array.from(new Set(rows.map((r) => r.division.trim()).filter(Boolean)));
      ...
      const existing = await db.select({ slug: committeeDivisions.slug }).from(committeeDivisions);
