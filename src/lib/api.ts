    ...
    try {
      ...
      if (parsed && typeof parsed === "object") {
        ...
        if (issues.length > 0) {
          return issues
            .map((i: any) => {
              const field = Array.isArray(i.path)
                ? i.path.join(".")
                : (i.path ?? "").replace(/^\//, "");
              const msg =
...
export const apiHelpers = {
  ...
  auth: {
    checkEmail: (email: string) => unwrap(api.auth["check-email"].post({ email } as never)),
  },
  ...
  registrations: {
    ...
    check: (query: string) => unwrap(api.registrations.check.post({ query } as never)),
  ...
  committeeDivisions: {
    ...
    reorder: (ids: number[]) => unwrap(api["committee-divisions"].reorder.put({ ids } as never)),
  ...
  galleryPhotos: {
    ...
    update: (id: string, body: unknown) => unwrap(api["gallery-photos"]({ id }).put(body as never)),
  ...
  journeyPhotos: {
    list: (journeyId: string) => unwrap(api["journey-photos"].get({ query: { journeyId } })),
  ...
  mediaPartners: {
    ...
    update: (id: string, body: unknown) => unwrap(api["media-partners"]({ id }).put(body as never)),
  ...
  certificates: {
    ...
    generateSingle: (body: unknown) =>
      unwrap(api.certificates["generate-single"].post(body as never)),
  },
  ...
  upload: async (file: File, options?: { isCommittee?: boolean }): Promise<{ url: string }> => {
    ...
    return api.upload.post({ file: processedFile } as never).then((res) => {
      if (res.error) {
  ...
  uploadCommittee: async (file: File): Promise<{ url: string }> => {
    ...
    return api.upload.committee.post({ file: processedFile } as never).then((res) => {
      if (res.error) {
        throw new Error(getApiError(res.error, "Upload gagal"));
      }
      return res.data as { url: string };
  ...
  uploadPlayerPhoto: async (file: File): Promise<{ url: string }> => {
    ...
    return api.upload["player-photo"].post({ file: processedFile } as never).then((res) => {
      if (res.error) {
