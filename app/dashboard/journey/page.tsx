...
export default function JourneyPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: (body: Record<string, unknown>) =>
      editingId ? apiHelpers.journeys.update(editingId, body) : apiHelpers.journeys.create(body),
    onSuccess: () => {
      setForm({
        year: "",
        theme: "",
  ...
  const handleSave = async () => {
    if (!form.year || !form.theme) {
      toast.error("Tahun dan tema wajib diisi");
      return;
    }
    setSaving(true);
    try {
  ...
  const handleDelete = (id: string, theme: string) => {
    setDeleteModal({
      title: "Hapus Journey",
      message: 'Yakin ingin menghapus "' + theme + '"?',
      onConfirm: async () => {
  ...
  return (
    <PageShell loading={loading}>
      ...
      {showAdd && (
        <SectionCard title={`${editingId ? "Edit" : "Tambah"} Journey`} bodyClassName="space-y-4">
          <FieldGroup className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <Field>
              ...
              <Input
                ...
                placeholder="22 - 24 Agustus 2026"
              />
            </Field>
            <Field>
              ...
              <Input
                type="number"
                value={form.competitionsCount}
                onChange={(e) => setForm({ ...form, competitionsCount: Number(e.target.value) })}
              />
            </Field>
      ...
      <div className="grid grid-cols-1 gap-3">
            ...
            <CardContent className="flex items-center justify-between gap-4 p-0">
              ...
              <div className="flex gap-1">
                ...
                <Button
                  variant="ghost"
                  size="icon-sm"
                  onClick={() => handleDelete(item.id, item.theme)}
                  aria-label="Hapus"
        ...
        {items.length === 0 && (
          <EmptyState
            icon={<Route />}
            title="Belum ada data journey."
            description="Tambah perjalanan ASTRO per tahun agar tampil di halaman profil."
          />
        )}
      </div>
      <Pagination
      ...
      {loading ? (
        ...
        <div className="mt-3 grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5">
          {photos.map((p) => (
            <div
              key={p.id}
              className="rounded-md group relative aspect-[4/3] overflow-hidden border border-border bg-muted"
            >
              <button
                type="button"
              ...
              >
                ...
                <img
                  ...
                  alt={p.caption || "Foto dokumentasi"}
        ...
        <p className="mt-2 text-11 text-muted-foreground">
          Belum ada foto dokumentasi. Upload untuk menambahkan.
        </p>
      )}
