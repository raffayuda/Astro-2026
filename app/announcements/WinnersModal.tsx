...
export default function WinnersModal({
  ...
  const allCertGroups = [
    ...(grouped["1"].length > 0
      ? [
          {
            name: "Juara 1",
            certs: grouped["1"].flatMap((w) => getCertsFor(w.id)),
            rank: "🥇 Juara 1",
          },
        ]
      : []),
    ...(grouped["2"].length > 0
      ? [
          {
            name: "Juara 2",
            certs: grouped["2"].flatMap((w) => getCertsFor(w.id)),
            rank: "🥈 Juara 2",
          },
        ]
      : []),
    ...(grouped["3"].length > 0
      ? [
          {
            name: "Juara 3",
            certs: grouped["3"].flatMap((w) => getCertsFor(w.id)),
            rank: "🥉 Juara 3",
          },
        ]
      : []),
  ...
  const anyGenerating =
    winnersWithoutCerts.length === 0 && winners.some((w) => generatingIds.has(w.id));
  ...
  return (
    <>
          ...
          <>
            ...
            <div className="mb-6 flex flex-col items-center justify-between gap-6 md:flex-row">
              <div className="flex-1 text-left">
                <Badge
                  variant="outline"
                  className="mb-3 gap-2 border-astro-cyan/20 bg-astro-cyan/10 px-3.5 py-1.5 text-xs font-black uppercase tracking-wider text-astro-cyan"
                >
                ...
                <p className="mt-2.5 max-w-xl text-xs leading-relaxed text-muted-foreground sm:text-sm md:text-base">
                  Terima kasih kepada seluruh peserta yang telah berpartisipasi dan menunjukkan
                  karya terbaiknya.
                </p>
              </div>
            ...
            <div className="mb-8 grid grid-cols-1 gap-3 rounded-2xl border border-astro-cyan/20 bg-astro-cyan/5 p-3.5 sm:grid-cols-2 md:p-4">
              <div className="flex items-center gap-3 px-2">
                ...
                <div className="min-w-0">
                  <span className="block text-10 font-bold uppercase tracking-wider text-muted-foreground">
                    Nama Lomba
                  </span>
              ...
              <div className="mb-6 grid grid-cols-1 items-end gap-6 pt-4 md:grid-cols-3 md:gap-4">
                {grouped["2"].length > 0 && (
                  <div className="relative flex h-full min-h-[200px] flex-col justify-between rounded-2xl border border-astro-cyan/20 bg-muted/40 px-4 pt-10 pb-5 text-center shadow-xs">
                    ...
                    {prizes.find(
                      (p) => p.label.toLowerCase().includes("2") || p.label === "Juara 2",
                    ) && (
                      <div className="mt-3 border-t border-astro-cyan-2/60 pt-3">
                        <span className="mb-1.5 block text-9 font-bold uppercase tracking-wider text-muted-foreground">
                          Hadiah
                        </span>
                        <ul className="space-y-1">
                          {splitPrizeItems(
                            ...
                            <li
                              key={i}
                              className="flex items-start justify-center gap-1.5 text-left"
                            >
                              <Check className="mt-0.5 size-3 flex-shrink-0 text-astro-cyan" />
                              <span className="text-11 font-bold leading-snug text-ink">
                                {item}
                              </span>
                            </li>
                          ))}
                ...
                {grouped["1"].length > 0 && (
                  <div className="relative flex h-full min-h-[220px] flex-col justify-between rounded-2xl border-2 border-amber-300 bg-linear-to-b from-amber-50/80 to-amber-100/40 px-4 pt-11 pb-6 text-center shadow-md ring-4 ring-amber-400/10 md:-translate-y-1">
                    ...
                    <div>
                      <span className="mb-1 block text-xs font-black uppercase tracking-widest text-amber-700">
                        Juara 1
                      </span>
                      {grouped["1"].map((w) => (
                        <h3
                          key={w.id}
                          className="text-lg font-black leading-snug text-foreground sm:text-xl"
                        >
                          {getWinnerName(w)}
                        </h3>
                      ))}
                    ...
                    {prizes.find(
                      (p) => p.label.toLowerCase().includes("1") || p.label === "Juara 1",
                    ) && (
                        ...
                        <ul className="space-y-1">
                          {splitPrizeItems(
                            ...
                            >
                              ...
                              <span className="text-xs font-bold leading-snug text-amber-900">
                                {item}
                              </span>
                            </li>
                          ))}
                ...
                {grouped["3"].length > 0 && (
                  <div className="relative flex h-full min-h-[200px] flex-col justify-between rounded-2xl border border-orange-200/80 bg-orange-50/30 px-4 pt-10 pb-5 text-center shadow-xs">
                    ...
                    <div>
                      <span className="mb-1 block text-xs font-bold uppercase tracking-wider text-amber-900/70">
                        Juara 3
                      </span>
                      {grouped["3"].map((w) => (
                        <h3
                    ...
                    {prizes.find(
                      (p) => p.label.toLowerCase().includes("3") || p.label === "Juara 3",
                    ) && (
                        ...
                        <ul className="space-y-1">
                          {splitPrizeItems(
                            ...
                            >
                              ...
                              <span className="text-11 font-bold leading-snug text-orange-900">
                                {item}
                              </span>
                            </li>
                          ))}
      ...
      {showCertModal && (
        <CertModal allCerts={allCertGroups} onClose={() => setShowCertModal(false)} />
      )}
    </>
  );
}
