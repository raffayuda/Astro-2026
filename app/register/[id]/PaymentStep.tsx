...
export default function PaymentStep({
  ...
  const [clientPaymentCode, setClientPaymentCode] = useState<string | null>(
    initialPaymentCode ?? null,
  );
  const [clientPaymentCodeType, setClientPaymentCodeType] = useState<string | null>(
    initialPaymentCodeType ?? null,
  );
    ...
    <div className="w-full min-w-0">
      <AnimatePresence mode="wait">
        {paymentStatus === "paid" ? (
          ...
          <motion.div key="failed" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
            <WindowCard title="Pembayaran" bodyClassName="gap-4">
              <XCircle className="size-10 text-astro-pink" aria-hidden />
              <h2 className="font-heading text-2xl font-black text-astro-navy">
                {paymentStatus === "expired" ? "Batas waktu habis" : "Pembayaran gagal"}
              </h2>
              <p className="text-sm leading-relaxed text-ink/75">
